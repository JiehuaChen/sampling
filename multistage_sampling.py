...
def sample(lowerlocationsx, lowerlocationsy, number_of_samples, district_name, tiffile_1k, upperrange, lowerrange, number_of_1_by_1=4, directory=''):
    ...
    ### specify grid resolution (grain, in m)
        ...
        select_prob_1k = map(lambda x: 0 if np.isnan(x) or x < lowerrange or x>upperrange else 1, crp_prob_grid)
...
if __name__ == '__main__':
    ...
    print sample([float(x) for x in sys.argv[1].split(',')], [float(y) for y in sys.argv[2].split(',')], int(sys.argv[3]), str(sys.argv[4]), str(sys.argv[5]), float(sys.argv[6]), float(sys.argv[7]))
